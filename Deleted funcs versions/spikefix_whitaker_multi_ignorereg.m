function [Xfixed, spike_summary] = spikefix_whitaker_multi_ignorereg(X, d, w, threshold,ignore_wn, plotit)
...
%     through all spectra in the  X data block, and this version ignores
%     specified regions.
...
%                  1st diff), Ignores given wavenumber regions.
%
% -------------------------------------------------------------------------
...
X_ignoredreg = saisir_replace_by_line(X,ignore_wn);
for ispec = 1:size(X.i,1)    
    [~, spike_pos] = spikefix_whitaker(X_ignoredreg.d(ispec,:), d, w,threshold,0);   % x, m, d, w, threshold, plotcheck
    [xfixed, spike_pos] = lincorr_spikes(X.d(ispec,:),d,spike_pos,0);
